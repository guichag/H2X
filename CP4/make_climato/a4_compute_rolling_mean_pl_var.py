"""Compute CP4 output pressure level variable rolling mean values"""
...
def get_file_pl_var_roll_mean(ds='CP4A', res=4, var='t_pl', year=2000, window=6, months=[3, 4, 5], lat_range=(-6., 6.), lon_range=(37., 50.)):
...
def load_pl_var_roll_mean(ds='CP4A', res=4, var='q2', year=2000, window=6, months=[3, 4, 5], lat_range=(-6., 6.), lon_range=(37., 50.)):
    """Load processed variable rolling mean values"""
    outfile = get_file_pl_var_roll_mean(ds, res, var, year, window, months, lat_range, lon_range)
    ...
    for y in years:
        ...
        for m in months:
            ...
            for fname in mfiles:
                ...
                data_sub = data.sel(latitude=slice(lat_min, lat_max), longitude=slice(lon_min, lon_max), pressure=lvls)
