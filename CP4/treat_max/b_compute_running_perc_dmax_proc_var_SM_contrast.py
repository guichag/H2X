"""Compute percentile of daily max Twb values on rolling N days windows"""
...
if __name__ == '__main__':
    ...
    for idx in months_days_idx:
        ...
        q_res = np.quantile(data, q, axis=0)
        q_out = q_res + mean.sel(days=idx)

        out_qs.append(q_out)

    out_qs = np.asarray(out_qs)
    out_qs = out_qs[int(qwindow / 2):-int(qwindow / 2)]

    out_qs = xr.DataArray(data=out_qs, dims=['days', 'latitude', 'longitude'], coords=dict(days=(["days"], months_days_idx[int(qwindow / 2):-int(qwindow / 2)]), latitude=(["latitude"], ydmax.latitude.values), longitude=(["longitude"], ydmax.longitude.values)))
